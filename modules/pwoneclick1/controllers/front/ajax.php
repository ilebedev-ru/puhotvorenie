    ...
    public function initContent()
    {
       ...
       /* if ($this->isXmlHttpRequest() === false) {
        ...
        if ($config['option_of_submit'] == '2') {
            ...
            $response['location'] = __PS_BASE_URI__ . 'order-confirmation?key=' . $this->context->customer->secure_key . '&id_cart=' . intval($this->context->cart->id) . '&id_module=' . intval($pwOneClick->id) . '&id_order=' . intval(Order::getOrderByCartId($this->context->cart->id));
        }
        $this->ajaxDie(Tools::jsonEncode($response));
    }
