    ...
    $(function(){
        $('#optform').submit(function(){
            $.ajax({
                ...
                url: '{/literal}{$link->getModuleLink('opt', 'opt')}{literal}' + '?rand=' + new Date().getTime(),
                async: true,
                cache: false,
                dataType : "json",
...
<div class="opt_page">
	<div class="opt_form">
		...
		<form class="std" method="POST" id="optform" action="{$link->getModuleLink('opt', 'opt')}">
			<p class="required form-group">
				<label for="fio">Ваше имя</label>
				<input class="form-control" required type="text" id="fio" name="fio"/>
			</p>
			<p class="required form-group">
	...
	<div class="opt_form">
		<form class="std" method="POST" id="optform" action="{$link->getModuleLink('opt', 'opt')}">
			<p><input type="submit" id="submitMessage" name="getPrice" value="Получить прайс-лист"></p>
		</form>
	</div>
</div>