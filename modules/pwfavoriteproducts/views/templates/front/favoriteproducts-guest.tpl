...
<div id="favoriteproducts_block_account">
	...
	{if $favoriteProducts}
		<div>
			...
			<div class="favoriteproduct clearfix">
				<a href="{$link->getProductLink($favoriteProduct.id)|escape:'html':'UTF-8'}" class="product_img_link">
					<img src="{$link->getImageLink($favoriteProduct.link_rewrite, $favoriteProduct.image, 'medium_default')|escape:'html':'UTF-8'}" alt=""/></a>
				<h3><a href="{$link->getProductLink($favoriteProduct.id)|escape:'html':'UTF-8'}">{$favoriteProduct.name|escape:'html':'UTF-8'}</a></h3>
				<div class="product_desc">{$favoriteProduct.description_short|strip_tags|escape:'html':'UTF-8'}</div>

				<div class="remove">
					<img rel="ajax_id_favoriteproduct_{$favoriteProduct.id}" src="{$img_dir}icon/delete.gif" alt="" class="icon" />
				</div>
			</div>
			{/foreach}
		</div>
	...
	<ul class="footer_links">
		<li class="fleft">
			<a href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}"><i class="fa fa-chevron-left"></i>{l s='Back to your account.' mod='favoriteproducts'}</a>
        </li>
	</ul>
</div>