...
class FianetSceau extends Module
{
    ...
    public function sendXML($id_order, $is_old_order = false)
    {
        ...
        $link = Tools::getShopDomainSsl(true, true). __PS_BASE_URI__ . 'modules/fianetsceau/commentsmanager.php?token=' . $token;
    ...
    public function hookNewOrder($params)
    {
        ...
        $this->insertDBElement(
            ...
            'customer_ip_address' => $this->context->cookie->customer_ip,
            'date' => $order->date_add),
            _DB_PREFIX_ . self::SCEAU_ORDER_TABLE_NAME
        );
    }
    ...
    /**
     * Add ip customer to session when cart is created or updated
     * @param type $params
     */
    ...
    public function getStar($nb_star, $max_star)
    {
        ...
        for ($i = 0; $i < $note; $i++) {
            $html .= "<img src='" . __PS_BASE_URI__ . 'modules/' .
                $this->name . '/views/img/star_on.png' . "' alt='' />";
        }
        if (!is_null($decimal)) {
            if (in_array($decimal, array('4', '5', '6'))) {
                $html .= "<img src='" . __PS_BASE_URI__ .
                    'modules/' . $this->name . '/views/img/star_half.png' . "' alt='' />";
        ...
        for ($i = 0; $i < $complement; $i++) {
            $html .= "<img src='" . __PS_BASE_URI__ . 'modules/' .
                $this->name . '/views/img/star_off.png' . "' alt='' />";
        }
        return $html;
    }
    ...
    public function saveSceauTables($table_temp_name, $table_name)
    {
        ...
        if ($exist) {
            Db::getInstance()->execute(
                'CREATE TABLE `' . _DB_PREFIX_ . $table_temp_name .
