...
class Yamodule extends PaymentModuleCore
{
    ...
    public function install()
    {
        ...
        $sql[] = 'CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'mws_product_size_chart`
            (
                `id_product` int(10) NOT NULL,
                `size_chart` varchar(10) NOT NULL,
    ...
    {
        if (Module::isInstalled('yamodule') && strpos($_SERVER['REQUEST_URI'], 'module/yamodule/')) {
            $_SERVER['REQUEST_URI'] = iconv('windows-1251', 'UTF-8', $_SERVER['REQUEST_URI']);
        }
    }
    ...
    public function makeData($product, $combination = false)
    {
        ...
        $unit = $this->getUnitName($product['id_product']);
        if ($product['features']) {
            foreach ($product['features'] as $feature) {
                ...
                if($feature['id_feature'] == Configuration::get('YA_MARKET_FEATURE_ID')){
                    $p['unit'] = $unit;
                }
                $params[] = $p;
            }
        ...
        if ($combination) {
            foreach ($combination['attributes'] as $id_attribute_group => $attr){
                if($id_attribute_group == Configuration::get('YA_MARKET_ATTRIBUTE_ID')){
                    $combination['attributes'][$id_attribute_group]['unit'] = $unit;
                }
            }
    ...
    protected function getUnitName($id_product)
    {
        return Db::getInstance()->getValue('SELECT `size_chart` FROM `'._DB_PREFIX_.'mws_product_size_chart` WHERE `id_product` = ' . $id_product);
    }
                ...
                foreach ($products as $product) {
                    ...
                    if (is_array($combinations) && count($combinations) > 0) {
                        ...
                        foreach ($combinations as $combination) {
                            ...
                            $comb_array[$combination['id_product_attribute']]['attributes'][$combination['id_attribute_group']] = array(
    ...
    public function selfPostProcess()
    {
        ...
        if (Tools::getIsset('add_size_chart')) {
            $this->setDefaultSizeCharts();
        }
    ...
    protected function setDefaultSizeCharts()
    {
        $size_chart = Tools::getValue('default_add_size_chart');
        Db::getInstance()->execute('TRUNCATE TABLE `'._DB_PREFIX_.'mws_product_size_chart`');
        $products = Db::getInstance()->executeS('SELECT id_product FROM `'._DB_PREFIX_.'product`');
        foreach ($products as $product)
        {
            Db::getInstance()->execute('INSERT INTO `'._DB_PREFIX_.'mws_product_size_chart` (`id_product`, `size_chart`)
                VALUES ('.$product['id_product'].', "'.$size_chart.'")');
        }
    }
    ...
    public function validateMarket()
    {
        ...
        Configuration::UpdateValue('YA_MARKET_FEATURE_ID', Tools::getValue('YA_MARKET_FEATURE_ID'));
        Configuration::UpdateValue('YA_MARKET_ATTRIBUTE_ID', Tools::getValue('YA_MARKET_ATTRIBUTE_ID'));
    ...
    public function getContent()
    {
        ...
        $vars_market = Configuration::getMultiple(array(
            ...
            'YA_MARKET_SET_GZIP',
            'YA_MARKET_DESC_TYPE',
            'YA_MARKET_FEATURE_ID',
            'YA_MARKET_ATTRIBUTE_ID',
        ));
    ...
    public function hookdisplayPayment($params)
    {
        ...
        $summary = $cart->getSummaryDetails(true);

        $total_to_pay = $cart->getOrderTotal(true, Cart::BOTH);

        if(!$cart->isAttachCartRule(5) & $summary['total_discounts'] == 0) {
			$cartRule=new CartRule(5);
			$percent=$cartRule->reduction_percent;
            $total_to_pay = $total_to_pay * ((100-$percent)/100);
        }

        $this->smarty->assign(array('summary' => $summary));
        ...
        if (Configuration::get('YA_ORG_INSIDE')) {
            ...
            if ($payments['YA_ORG_PAYMENT_YANDEX']) {
                ...
                $newOption->setModuleName($this->name)
                    ->setCallToActionText($this->l('Payment from the purse in Yandex.Money.'))
                    ->setAction($this->context->link->getModuleLink($this->name, 'redirectk', array('type' => 'PC'), true));
            ...
            if ($payments['YA_ORG_PAYMENT_CARD']) {
                $newOption = new PrestaShop\PrestaShop\Core\Payment\PaymentOption();
                $newOption->setModuleName($this->name)
                    ->setCallToActionText($this->l('Arbitrary payment with Bank card.'))
                    ->setAction($this->context->link->getModuleLink($this->name, 'redirectk', array('type' => 'AC'), true));
                $payment_options[] = $newOption;
            }
            ...
            if ($payments['YA_ORG_PAYMENT_MOBILE']) {
                $newOption = new PrestaShop\PrestaShop\Core\Payment\PaymentOption();
                $newOption->setModuleName($this->name)
                    ->setCallToActionText($this->l('Payment with mobile phone account.'))
                    ->setAction($this->context->link->getModuleLink($this->name, 'redirectk', array('type' => 'MC'), true));
                $payment_options[] = $newOption;
            }
            ...
            if ($payments['YA_ORG_PAYMENT_WEBMONEY']) {
                $newOption = new PrestaShop\PrestaShop\Core\Payment\PaymentOption();
                $newOption->setModuleName($this->name)
                    ->setCallToActionText($this->l('Payment of the purse in system.'))
                    ->setAction($this->context->link->getModuleLink($this->name, 'redirectk', array('type' => 'WM'), true));
                $payment_options[] = $newOption;
            }
            ...
            if ($payments['YA_ORG_PAYMENT_TERMINAL']) {
                $newOption = new PrestaShop\PrestaShop\Core\Payment\PaymentOption();
                $newOption->setModuleName($this->name)
                    ->setCallToActionText($this->l('Payment in cash through cash desks and terminals.'))
                    ->setAction($this->context->link->getModuleLink($this->name, 'redirectk', array('type' => 'GP'), true));
                $payment_options[] = $newOption;
            }
            ...
            if ($payments['YA_ORG_PAYMENT_SBER']) {
                $newOption = new PrestaShop\PrestaShop\Core\Payment\PaymentOption();
                $newOption->setModuleName($this->name)
                    ->setCallToActionText($this->l('Payment via Sberbank: payment by SMS or Sberbank Online.'))
                    ->setAction($this->context->link->getModuleLink($this->name, 'redirectk', array('type' => 'SB'), true));
                $payment_options[] = $newOption;
            }
            if ($payments['YA_ORG_PAYMENT_ALFA']) {
                $newOption = new PrestaShop\PrestaShop\Core\Payment\PaymentOption();
                $newOption->setModuleName($this->name)
                    ->setCallToActionText($this->l('Payment via Alfa-Click.'))
                    ->setAction($this->context->link->getModuleLink($this->name, 'redirectk', array('type' => 'AB'), true));
                $payment_options[] = $newOption;
            }
            if ($payments['YA_ORG_PAYMENT_PB']) {
                $newOption = new PrestaShop\PrestaShop\Core\Payment\PaymentOption();
                $newOption->setModuleName($this->name)
                    ->setCallToActionText($this->l('Payments via Promsvyazbank.'))
                    ->setAction($this->context->link->getModuleLink($this->name, 'redirectk', array('type' => 'PB'), true));
                $payment_options[] = $newOption;
            }
            if ($payments['YA_ORG_PAYMENT_MA']) {
                $newOption = new PrestaShop\PrestaShop\Core\Payment\PaymentOption();
                $newOption->setModuleName($this->name)
                    ->setCallToActionText($this->l('Payment via MasterPass.'))
                    ->setAction($this->context->link->getModuleLink($this->name, 'redirectk', array('type' => 'MA'), true));
                $payment_options[] = $newOption;
            }
            ...
            if ($payments['YA_ORG_PAYMENT_QW']) {
                $newOption = new PrestaShop\PrestaShop\Core\Payment\PaymentOption();
                $newOption->setModuleName($this->name)
                    ->setCallToActionText($this->l('Payment via QIWI Wallet.'))
                    ->setAction($this->context->link->getModuleLink($this->name, 'redirectk', array('type' => 'QW'), true));
                $payment_options[] = $newOption;
            }
            if ($payments['YA_ORG_PAYMENT_QP']) {
                $newOption = new PrestaShop\PrestaShop\Core\Payment\PaymentOption();
                $newOption->setModuleName($this->name)
                    ->setCallToActionText($this->l('Payment through a trusted payment (Kuppi.ru).'))
                    ->setAction($this->context->link->getModuleLink($this->name, 'redirectk', array('type' => 'QP'), true));
                $payment_options[] = $newOption;
            }
        } else {
            $newOption = new PrestaShop\PrestaShop\Core\Payment\PaymentOption();
            $newOption->setModuleName($this->name)
                ->setCallToActionText($this->l('Оплатить через Яндекс.'))
                ->setAction($this->context->link->getModuleLink($this->name, 'redirectk', array('type' => 'QW'), true));
    ...
    public function hookActionAdminProductsControllerSaveAfter($params){
        ...
        if ($product->id AND Tools::getValue('product_size_chart')) {
            $unit = Tools::getValue('product_size_chart');
            Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'mws_product_size_chart` WHERE `id_product` = ' . $product->id);
            Db::getInstance()->execute('INSERT INTO `'._DB_PREFIX_.'mws_product_size_chart` (`id_product`, `size_chart`)
                VALUES ('.$product->id.', "'.$unit.'")');
        }
    }

    public function hookDisplayAdminProductsExtra($params){
        $id_product = Tools::getValue('id_product');
        $unit = Db::getInstance()->getValue('SELECT `size_chart` FROM `'._DB_PREFIX_.'mws_product_size_chart` WHERE `id_product` = ' . $id_product);
        if(empty($unit))
            $unit = 'INT';
        $this->context->smarty->assign(array(
            'unit' => $unit
