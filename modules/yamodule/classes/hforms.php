...
class Hforms
{
            ...
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        ...
                        'class' => 't',
                        'type' => 'text',
                        'name' => 'default_add_size_chart',
                        'label' => $this->l('Размерная сетка по умолчанию'),
                    ),
                    array(
                        'type' => 'select',
                        'label' => $this->l('Характеристика указывающая на рамер'),
                        'name' => 'YA_MARKET_FEATURE_ID',
                        'options' => array(
                    ...
                    array(
                        'type' => 'select',
                        'label' => $this->l('Атрибут указывающий на рамер'),
                        'name' => 'YA_MARKET_ATTRIBUTE_ID',
                        'options' => array(
                ...
                'buttons' => array(
                    ...
                    'add_size_chart' => array(
                        'title' => $this->l('Установить размерную сетку'),
                        'name' => 'add_size_chart',
                        'type' => 'submit',
                        'class' => 'btn btn-default pull-right',
                        'icon' => 'process-icon-partial_refund'
                    ),
                )
            ),
        );
    ...
    {
        $features = Db::getInstance()->executeS("SELECT `id_feature`, `name` FROM `"._DB_PREFIX_."feature_lang` ORDER BY `name`");

        return array_merge(array(array('id_feature' => 0, 'name' => 'Выбрать характеристику')), $features);
    }

    protected function getAttributes()
    {
        $attributes =  Db::getInstance()->executeS("SELECT `id_attribute_group`, `name` FROM `"._DB_PREFIX_."attribute_group_lang` ORDER BY `name`");

        return array_merge(array(array('id_attribute_group' => 0, 'name' => 'Выбрать атрибут')), $attributes);
    }
