...
class AdminCertissimController extends AdminOrders
{
    ...
    {
        $link_eval = 'index.php?tab=AdminCertissim&action=getAllWaitingScores&token='
        ...
        $link_reeval = 'index.php?tab=AdminCertissim&action=getReevaluations&token='
            . Tools::getAdminTokenLite('AdminCertissim');
        ...
        $header = '<fieldset>
			...
			<a href="' . $link_eval . '"><img src="' . $img_eval . '"/>' . $this->l('Get all waiting evaluations') . '</a></div>
				<div class=\'certissim_control\'><a href="' . $link_reeval
            . '"><img src="' . $img_reeval . '"/>'
            . $this->l('Get all reevaluations') . '</a></div>
					</div></fieldset>';
    ...
    public function display()
    {
        if (Tools::isSubmit('action') && Tools::getValue('action') == 'viewLog') {
            ...
            $url_back = 'index.php?tab=AdminModules&configure=fianetfraud
                &tab_module=payment_security&module_name=fianetfraud
            ...
            echo '<center><a style=\'padding: 3px; margin: 6px; border: 1px solid black\' 
				...
				<textarea style=\'margin: 6px; cols=\'160\' rows=\'50\' readOnly\'>'
                . $log_txt . '</textarea><div class=\'clear\'></div>
