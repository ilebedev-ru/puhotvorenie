...
class CertissimSac extends CertissimService
{
     ...
     * @return string script response
     */
    public function sendStacking(CertissimXMLElement $stack, $site_id = null)
    {
        /*	 * ************* MODIF FRANFINANCE *************** */
    $data = array(
        'siteid' => !is_null($site_id) ? $site_id : $this->getSiteId(),
    ...
    public function generateAuthKey(CertissimXMLElement $xml)
    {
        $key = $this->getAuthKey();
        return hash_hmac("sha1", $xml->getXML(), pack('H*', $key), false);
    }

    public function sendCopilot(CertissimXMLElement $xml)
    {
        $data = array(
        'siteid' => $this->getSiteId(),
        'auth' => $this->generateAuthKey($xml),
        'payload' => $xml->getXML(),
    );

        $result_copilot = array();
        $con = new CertissimFianetSocket($this->getUrlcopilot(), 'POST', $data);
        return $con->send_copilot();
    }
}
