...
require_once _PS_MODULE_DIR_.'pwoneclick/pwoneclick.php';
...
class PwOneClickAjaxModuleFrontController extends ModuleFrontController
{
    ...
    public function initContent()
    {
       ...
       /* if ($this->isXmlHttpRequest() === false) {
        ...
        if (Tools::getValue('getCustomer')) {
            ...
            return $this->ajaxDie($response);
        }

        $name           = Tools::getValue(self::INPUT_NAME);
        $phone          = Tools::getValue(self::INPUT_PHONE);
        $email          = Tools::getValue(self::INPUT_EMAIL);
        $name           = 'Клиент';
        ...
        # Проверяем на непустоту иначе валидный телефон получается.
        if ( !empty($phone) ) {
            if (!Validate::isPhoneNumber($phone)) {
                $this->addError(Array(self::INPUT_PHONE => $this->module->l('Не корректный телефон', 'ajax')));
            }
        } else {
            $this->addError(Array(self::INPUT_PHONE => $this->module->l('Не корректный телефон', 'ajax')));
        }
        ...
        if ( empty($email) || !Validate::isEmail($email)) {
            $this->addError(Array(self::INPUT_EMAIL => $this->module->l('Не корректная почта', 'ajax')));
        }

        if ( empty($name) || !Validate::isName($name)) {
            $this->addError(Array(self::INPUT_NAME => $this->module->l('Не корректное имя', 'ajax')));
        }
        ...
        if ($this->customer->isLogged() === false || $this->customer->email != $email) {
            ...
            if ($result['id_customer'] && $email) {
                $this->customer = new Customer($result['id_customer']);
        ...
        if ( !$this->issetErrors()) {
            ...
            if ($id_product && $this->isExistProduct($id_product)) {
                //d([$id_product, $combination, $quantity]);
        ...
        $config = Configuration::get('PW_ONE_CLICK');
        $config = unserialize($config);
        // if ($config['option_of_submit'] == '2') {

            $pwOneClick = new PwOneClick();

            $response['location'] = __PS_BASE_URI__ . 'order-confirmation?key=' . $this->context->customer->secure_key . '&id_cart=' . intval($this->context->cart->id) . '&id_module=' . intval($pwOneClick->id) . '&id_order=' . intval(Order::getOrderByCartId($this->context->cart->id)) . '&pwexpressorder=1';
        // }
    ...
    private function getAddress($id = 0)
    {
        if ($id === 0) {
            ...
            //$address->phone_mobile  = Tools::getValue(self::INPUT_PHONE);
