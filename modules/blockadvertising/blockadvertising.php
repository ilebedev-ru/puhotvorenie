...
class BlockAdvertising extends Module
{
	...
	protected function initialize()
	{
		...
		$this->adv_btn = htmlentities(Configuration::get('BLOCKADVERT_BTN'), ENT_QUOTES, 'UTF-8');
	}
	...
	public function install()
	{
		...
		Configuration::updateGlobalValue('BLOCKADVERT_BTN', 'PrestaShop');
	...
	public function uninstall()
	{
		...
		Configuration::deleteByName('BLOCKADVERT_BTN');
	...
	public function postProcess()
	{
		...
		if (Tools::isSubmit('submitAdvConf'))
		{
			...
			// If the btn is not set, then delete it in order to use the next default value (either the global value or the group value)
			if ($title = Tools::getValue('adv_btn'))
				Configuration::updateValue('BLOCKADVERT_BTN', $title);
			elseif (Shop::getContext() == Shop::CONTEXT_SHOP || Shop::getContext() == Shop::CONTEXT_GROUP)
				Configuration::deleteFromContext('BLOCKADVERT_BTN');
		...
		$fields_form = array(
			'form' => array(
				...
				'input' => array(
					...
					array(
						'type' => 'text',
						'label' => $this->l('Title of the btn'),
						'name' => 'adv_btn',
						'desc' => $this->l('This title will be displayed on btn')
					),
				),
				'submit' => array(
					'title' => $this->l('Save'),
	...
	public function getConfigFieldsValues()
	{
		return array(
			...
			'adv_btn' => Tools::getValue('adv_btn', Configuration::get('BLOCKADVERT_BTN')),
		);
	}
}
