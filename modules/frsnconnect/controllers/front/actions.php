...
class FrSnConnectActionsModuleFrontController extends ModuleFrontController
{
    ...
    public function init()
    {
        ...
        $this->display_column_rigth = false;
    ...
    public function processCreateAccount()
    {
        if (sizeof($_REQUEST)) {
            try {
                ...
                if (!isset($_POST['email']) || empty($_POST['email']) || $_POST['email'] == Configuration::get('FRSNCONN_EMPTYEMAIL'))
                    $_POST['email'] = md5(uniqid()).'@mail.com';
                ...
                $_POST['firstname'] = Tools::getValue('customer_firstname');// . ' ' . Tools::getValue('customer_lastname');
                    ...
                    if (!count($this->errors)) {
                        ...
                        if (!count($this->errors))
                        {
                            if (!$customer->add())
                            {
								...
								$addressObj->alias = 'New Address';
                                $addressObj->id_customer = $customer->id;
                                $addressObj->id_country = Configuration::get('PS_COUNTRY_DEFAULT');
								$addressObj->lastname = Tools::getValue('customer_lastname', Tools::getValue('customer_firstname'));
								$addressObj->firstname = Tools::getValue('customer_firstname', Tools::getValue('customer_lastname'));
                                $addressObj->address1 = 'Your address';
                                $addressObj->city = Tools::getValue('city', 'Москва');
                                $addressObj->save();
    ...
    public function processAccAuth()
    {
        if (Tools::isSubmit('state')) {
			$st = $_REQUEST['state']?$_REQUEST['state']:$_GET['state'];
            $_REQUEST[$st] = 1;
            $_GET[$st] = 1;
        }
        if (sizeof($_REQUEST)) {
...
//                if (SNTools::IsDebug())  
//                    Logger::AddLog($this->module->name.':'.basename(__FILE__).':'.__LINE__.' back='.$back, 1);
                ...
                if (isset($sn_id) AND $sn_id > 0) {
                    ...
                    if (isset($service) AND $back)
                        $service->setState('back', $back);

                    if (isset($service) AND $service->authenticate()) {
                        if (!$back AND $service->hasState('back')) {
                            $back = $service->getState('back');
                        }
                        ...
                        if ($authentication OR $customer->id) {
                           ...
                           $_POST['lastname'] = (isset($service->lastname)) ? $service->lastname : $service->name;
                            ...
                            if ($PS_REGISTRATION_PROCESS_TYPE) {
                                ...
                                $_POST['city'] = (isset($service->city)) ? $service->city : 'Москва';
    ...
    public function initContent()
    {
        ...
        if (Tools::getValue('accTest')) {
            $cc = $this->context->cookie;
            $cc = print_r($cc, true);
            $this->context->smarty->assign('cooock', $cc);
            $this->display_column_right = false;
            $this->display_header = false;
            $this->display_footer = false;

            $this->setTemplate('frsnconnect-test.tpl');
            return;
        }
    ...
    public function setMedia()
    {
        ...
        parent::setMedia();
        $this->addCSS($this->module->getLocalPath() . 'frsnconnect.css');
