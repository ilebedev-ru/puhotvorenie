...
abstract class frOAuthSrv extends frSNSrvBase { 
    ...
    public function authenticate() {
        ...
        if (isset($_REQUEST['oauth_token']) || isset($_GET['oauth_token'])) 
            $oauthToken = $_REQUEST['oauth_token'] ? $_REQUEST['oauth_token'] : $_GET['oauth_token'];
        
        if (isset($_REQUEST['oauth_verifier']) || isset($_GET['oauth_verifier'])) 
            $oauthVerifier = $_REQUEST['oauth_verifier'] ? $_REQUEST['oauth_verifier'] : $_GET['oauth_verifier'];
...
//        return $url;  
    ...
    private static function GetTokenFromUrl($url) {
        ...
        if ($headers['http_code'] != 200) 
            $error = $response; //not work in static method
