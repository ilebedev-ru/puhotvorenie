...
class DirectLink
{
    ...
    public function request($request, array $data)
    {
        ...
        $data['SHASIGN'] = $this->getShaSign($data, $this->getShaInPassphrase());
     ...
     * @return string
     */
    public function getShaSign(array $data, $passphrase)
    {
        if ($passphrase == '') {
        ...
        if ($this->getDebug()) {
            print PHP_EOL . $signature;
            print PHP_EOL . Tools::strtoupper(sha1($signature));
        }
