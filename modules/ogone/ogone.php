...
class Ogone extends PaymentModule
{
     ...
     * @var array
     */
    protected $ignore_key_list = array('secure_key', 'ORIG', 'controller');
    ...
    {
        $result = Configuration::updateValue('OGONE_ALIAS_PM', 'CreditCard') &&
    ...
    public function installTabs()
    {
        ...
        $tabs_to_add = array(
            'AdminOgoneTransactions' => $this->l('Ogone Transactions'),
            'AdminOgoneOrders' => $this->l('Ogone Orders'),
        );
    ...
    public function installDBTables()
    {
        ...
        if (!Db::getInstance()->Execute('
          CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'ogone_alias` (
              ...
              `date_add` datetime NOT NULL,
              `date_upd` datetime NOT NULL,
              PRIMARY KEY (`id_ogone_alias`), KEY(`id_customer`), KEY(`alias`), KEY(`active`), KEY(`expiry_date`)
        ) ENGINE=' . _MYSQL_ENGINE_ . ' DEFAULT CHARSET=utf8')) {
            return false;
        }
        ...
        if (!Db::getInstance()->Execute('
          CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'ogone_pm_shop` (
              ...
              `position` INT(10) NOT NULL DEFAULT 0,
              `active` INT(10) NOT NULL DEFAULT 0,
              PRIMARY KEY (`id_ogone_pm`), KEY(`id_shop`), KEY(`id_shop_group`), KEY(`active`), KEY(`position`)
        ) ENGINE=' . _MYSQL_ENGINE_ . ' DEFAULT CHARSET=utf8')) {
            return false;
        }
    ...
    public function addDefaultPaymentModes()
    {
        ...
        foreach ($this->getDefaultPaymentModes() as $idx => $row) {
             ...
             */
            if ($pm->save() && Validate::isLoadedObject($pm)) {
                if ($shops) {
    ...
    public function hookDisplayAdminOrder($params)
    {
        ...
        $controller = (version_compare(_PS_VERSION_, '1.5', 'lt') ? 'AdminOgoneOrders14' : 'AdminOgoneOrders');
        $tpl_vars = array(
            'can_use_direct_link' => $this->canUseDirectLink(),
            'can_capture' => $can_capture,
            'cc_title' => $can_capture ? $this->l('Capture') : $error,
            'capture_link' => $can_capture ? 'index.php?controller='.$controller.'&id_order=' . (int) $id_order .
                '&action=capture&token=' . Tools::getAdminTokenLite($controller) : '',
        );
    ...
    {
        if (Tools::getValue('configure', Tools::getValue('module_name')) == $this->name) {
            if (version_compare(_PS_VERSION_, '1.5', '<')) {
                $result = array();
    ...
    public function hookPayment($params)
    {
        ...
        if (Configuration::get('OGONE_USE_PM')) {
            foreach ($this->getPaymentModesList() as $pm) {
                if (!$pm->active) {
                    continue;
                }
    ...
    {
        $tpl = (version_compare(_PS_VERSION_, '1.6', 'ge') ? 'ogone16' : 'ogone');
        if ($type) {
            $tpl .= '-' . $type;
        }
        $tpl .= '.tpl';
    ...
    public function hookOrderConfirmation($params)
    {
        ...
        if ($params['objOrder']->valid ||
            (Configuration::get('OGONE_OPERATION') == self::OPERATION_AUTHORISE &&
                (int) $params['objOrder']->current_state === (int) Configuration::get('OGONE_PAYMENT_AUTHORIZED'))) {
    ...
    protected function processConfig()
    {
        ...
        if (Tools::getValue('action')) {
            ...
            if ($action === 'delete_pm') {
                ...
                if ($this->deletePM((int) Tools::getValue('pmid'))) {
                    $messages['successes'][] = $this->l('Payment mean deleted');
                } else {
                    $messages['errors'][] = $this->l('Unable to delete payment mean');
                }
            }
        }
        ...
        if (Tools::isSubmit('submitOgonePM')) {
            ...
            if ($this->updatePMConfiguration()) {
                $messages['successes'][] = $this->l('Payment means updated');
            } else {
                $messages['errors'][] = $this->l('Unable update payment means');
            }
        ...
        if (Tools::isSubmit('submitOgoneAddPM')) {
            $this->selected_tab = 'pm';
            $pm = $this->addPaymentMean();
            if ($pm) {
                $messages['successes'][] = $this->l('Payment mean was added');
                list($image_upload, $message) = $this->uploadPaymentLogo(
                    'add_pm_logo',
                ...
                if ($image_upload === null) {
                    ...
                    $messages['successes'][] = $this->l('Payment mean was added with image');
                } else {
                    $messages['warnings'][] = $this->l('Payment mean was added but') . ' ' . lcfirst($message);
                }

            } else {
                $messages['errors'][] = $this->l('Unable to add payment mean');
            }
    ...
    protected function addPaymentMean()
    {
        ...
        foreach ($languages as $language) {
            ...
            $name[$id_lang] = Tools::getValue('add_pm_name_' . $id_lang);
        ...
        $payment_mean = new OgonePM();
        $payment_mean->brand = $brand;
        $payment_mean->pm = $pm;
        $payment_mean->active = 1;
        $payment_mean->name = $name;
        $payment_mean->description = $description;
    ...
    protected function getPaymentModesFormHtml()
    {
        ...
        $tpl_vars = array(
            'payment_means' => $this->getPaymentModesList(),
    ...
    protected function getConfigurationVariables()
    {
        return array(
            ...
            'OGONE_ALIAS_PM' => Configuration::get('OGONE_ALIAS_PM'),
            'OGONE_DL_TIMOEUT' => Configuration::get('OGONE_DL_TIMOEUT'),
    ...
    protected function updateConfiguration()
    {
        ...
        Configuration::updateValue('OGONE_ALIAS_PM', Tools::getValue('OGONE_ALIAS_PM'));
        ...
        Configuration::updateValue('OGONE_DL_TIMOEUT', (int) Tools::getValue('OGONE_DL_TIMOEUT'));
    ...
    protected function assignPaymentVars($params)
    {
        ...
        $ogone_params['DECLINEURL'] = $this->getConfirmationUrl();
        $ogone_params['EXCEPTIONURL'] = $this->getConfirmationUrl();
        ...
        if (Configuration::get('OGONE_USE_ALIAS') && isset($params['alias'])) {
            $ogone_params['ALIAS'] = $params['alias']['alias'];
            $ogone_params['ALIASUSAGE'] = $this->l('You can use payment mean stored on secure server');
    ...
    public function getKlarnaVars($cart)
    {
        ...
        /* if some parameters are empty, this payement canot be used */
        ...
        if ($total_shipping > 0) {
            $products[] = array(
                ...
                'quantity' => 1,
                'tax_rate' => $cart->carrier_tax_rate,
            );
        }
        $idx = 1;
        foreach ($products as $product) {
            $product_row = array(
                ...
                'ITEMPRICE' => round($product['quantity'] * $product['price_with_reduction'], 2) / $product['quantity'],
    ...
    public function calculateShaSign($ogone_params, $sha_key)
    {
        ...
        $this->log($sha_key);
        $this->log($shasign);
        $this->log(Tools::strtoupper(sha1($shasign)));
        return Tools::strtoupper(sha1($shasign));
    }

    protected function getPaymentModesList()
    {
        $pm_tpl_list = array();
    ...
    {
        return Tools::getProtocol() . $_SERVER['HTTP_HOST'] . _MODULE_DIR_ . 'ogone/confirmation.php';
    }
    /**
    ...
    public function log($data)
    {
        if (Configuration::get('OGONE_USE_LOG')) {
            if ($this->log_file === null) {
                ...
                $this->log('Prestashop : ' . _PS_VERSION_);
                $this->log('Module : ' . $this->version);
            }
            ...
            $message = sprintf('[%s] [%s] %s %s', $source, $timestamp, $data, PHP_EOL);
    ...
    protected function getLogFilesData()
    {
        ...
        foreach ($this->getLogFiles() as $log_file) {
            ...
            $result[$log_name] = array(
                ...
                'dt' => date('Y-m-d H:i:s', filemtime($log_file)),
                ...
                if (isset($response['NCERROR']) && !empty($response['NCERROR'])) {
                    ...
                    try {
                        ...
                        if ($response_status === DirectLink::STATUS_CAPTURED) {
                             ...
                             */
                            $order->total_paid_real = round($response['AMOUNT'], $precision);
                            if ($order->update()) {
                                $update_history = true;
                            } else {
     ...
     */
    public function getHostedTokenizationPageRegistrationUrl($id_customer)
    {
        ...
        if (version_compare(_PS_VERSION_, '1.5', 'ge')) {
            $accept_url = $this->context->link->getModuleLink(
                $this->name,
                'aliases',
                array('result' => 'ok', 'alias' => $alias),
                true
            );
            $exception_url = $this->context->link->getModuleLink(
                $this->name,
                'aliases',
                array('result' => 'ko', 'alias' => $alias),
                true
            );
        } else {
            ...
            htmlspecialchars($_SERVER['HTTP_HOST'], ENT_COMPAT, 'UTF-8').
            __PS_BASE_URI__.'modules/ogone/aliases.php';
            $accept_url = $base_url.'?result=ok&alias='.$alias;
            $exception_url = $base_url.'?result=ko&alias='.$alias;
        }

        $query = array(
            'ACCOUNT.PSPID' => Configuration::get('OGONE_PSPID'),
            'ALIAS.ALIASID' => $alias,
            'CARD.PAYMENTMETHOD' => $this->getAliasPM(),
    ...
    protected function getAliasPM()
    {
        switch (Configuration::get('OGONE_ALIAS_PM')) {
            case 'DirectDebit':
                $result = 'DirectDebit';
                break;
            case 'CreditCard':
    ...
    public function createAlias($id_customer, $data)
    {
        ...
        if (!array_key_exists('SHASIGN', $data)) {
        ...
        if (OgoneAlias::getByAlias($data['ALIAS'])) {
    ...
    {
        $path =  array('views', 'img', Tools::strtolower($alias['brand'] . '.png'));
