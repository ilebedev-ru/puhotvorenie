...
class leobtslider extends Module
{
	...
	private function installSamples()
	{
		...
		for ($i = 1; $i <= 3; ++$i)
		{
			...
			foreach ($languages as $language)
			{
				...
				$slide->image[$language['id_lang']] = 'sample-'.$i.'.png';
			}
			$slide->add();
		}
	}
	...
	private function _displayAddForm()
	{
		...
		foreach ($languages as $language)
		{
			...
			/* Display image */
			if ($slide && $slide->image[$language['id_lang']])
				$this->_html .= '<img src="'.__PS_BASE_URI__.'modules/'.$this->name.'/images/'.$slide->image[$language['id_lang']].'" width="'.($this->getParams()->get('imgwidth',960)/2).'" height="'.($this->getParams()->get('imgheight',585)/2).'" alt=""/>';
			$this->_html .= '</div>';
		}
