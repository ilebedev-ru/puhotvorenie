...
function startCDEK(init_only){
    ...
    function initCDEK()
    {
        ...
        ajaxGetListPvz = $.ajax({
            ...
            success: function (r)
            {
                ...
                // if (r.cdek_carriers.length){
                //     cdek_carriers = r.cdek_carriers;
                // }

                $.each(r.cdek_carriers, function (id_carrier, carrier) {
                    var is_ps_17 = false;
                    ...
                    if (delivery_option_elem.length)
                    {
                        ...
                        function insertDelay() {
                            if (is_ps_17) {
                                ...
                                $delay = $delay.replace(new RegExp('Дата доставки'), '</br>'+'Дата доставки');
                                there_insert.find('.d-item .descinfo .delay').html($delay);
                            }
                        }
                        ...
                        delivery_date = carrier.delay.match(/(\d{4}-\d{2}-\d{2})/);
                        cdek_order_info.date = delivery_date[0];
                        initDeliveryTime();
    ...
    function initDeliveryTime() {
        ...
        cdek_block.find('.cdek_delivery_date').datepicker({
            dateFormat: "dd-mm-yy",
            minDate: new Date(cdek_order_info.date)
        }).val(cdek_order_info.date);

        if (cdek_block.find('.cdek_time_slider').length) {
            cdek_block.find('.cdek_time_slider').rangeSlider({
                range: {min: 0, max: 23},
                bounds: {min: 0, max:23},
                defaultValues:{
            ...
            cdek_block.find('.cdek_delivery_time_begin').val(parseInt(cdek_order_info.time_begin));
            cdek_block.find('.cdek_delivery_time_end').val(parseInt(cdek_order_info.time_end));

            cdek_block.find('.cdek_time_slider').bind("valuesChanged", function(e, data){
                cdek_block.find('.cdek_delivery_time_begin').val(parseInt(data.values.min));
                cdek_block.find('.cdek_delivery_time_end').val(parseInt(data.values.max));
                updateCdekDate();
            });
        } else {
            cdek_order_info.time_begin = 0;
            cdek_order_info.time_end = 23;
        }
    }
            ...
            '/modules/cdek/search.php', {
                ...
                parse: function(data) {
                    ...
                    for (var i = 0; i < data.length; i++){
                        mytab[mytab.length] = { data: data[i]['PostCodeList'], value: data[i]['FullName']};
                    }
                    return mytab;
                }
            })
        .result(function(event, data, formatted) {
            $("input[name='city']").val(formatted);
            $("input[name='postcode']").val(data);
        });
}