...
function _posts()
{
    ...
    if ($post['comment_status'] == 1) {
        ...
        if (empty($name)) {
            $array_error['name'] = $SmartBlog->l('Имя необходимо');
        }
        if (empty($post)) {
            $array_error['post'] = $SmartBlog->l('Организация необходима');
        }
        if (empty($comment)) {
            $array_error['comment'] = $SmartBlog->l('Комментарий должен быть от 25 до 1500 символов!');
        }
        ...
        if (is_array($array_error) && count($array_error)) {
            ...
            if ($bc->add()) {
                $array_success['common'] = $SmartBlog->l('Ваш комментарий успешно добавлен.');
                $array_success['success'] = $SmartBlog->l('Ваш комментарий успешно добавлен.');
