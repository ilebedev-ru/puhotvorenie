...
class BlogPostCategory extends ObjectModel
{
    ...
    public static function getToltalByProduct($id_lang, $id_product)
    {
        $sql = 'SELECT * FROM ' . _DB_PREFIX_ . 'smart_blog_post_lang pl, ' . _DB_PREFIX_ . 'smart_blog_post p, ' . _DB_PREFIX_ . 'smart_blog_product_related pr
                WHERE pl.id_lang=' . $id_lang . ' and p.active = 1 AND pl.id_smart_blog_post=p.id_smart_blog_post AND  pr.id_post = pl.id_smart_blog_post AND pr.id_product = ' . $id_product . '
                ORDER BY p.id_smart_blog_post DESC';

        if (!$posts = Db::getInstance()->executeS($sql))
            return false;

        $result = self::composePostsResult($posts);
        return $result;
    }

    public static function getToltalByCategoryProduct($id_lang, $id_category)
    {
        $result = array();
        $sql = 'SELECT * FROM ' . _DB_PREFIX_ . 'smart_blog_post_lang pl, ' . _DB_PREFIX_ . 'smart_blog_post p, ' . _DB_PREFIX_ . 'smart_blog_category_of_product_related pr
                WHERE pl.id_lang=' . $id_lang . ' and p.active = 1 AND pl.id_smart_blog_post=p.id_smart_blog_post AND  pr.id_post = pl.id_smart_blog_post AND pr.id_category = ' . $id_category . '
                ORDER BY p.id_smart_blog_post DESC';
     ...
     * @return mixed
     */
    public static function composePostsResult($posts)
    {
        $result = array();
        $i = 0;
        $BlogCategory = new BlogCategory();
