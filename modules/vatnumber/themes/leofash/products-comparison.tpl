                ...
                <td width="{$width}%" class="ajax_block_product product-block comparison_infos">
                    <div class="product-container">
                        ...
                        <div class="product_meta">
                            <p class="name"><a href="{$product->getLink()}"
                        ...
                        <div class="buttons-action product_bottom">
                            <a class="cmp_remove clearfix button"
                            ...
                            {if (!$product->hasAttributes() OR (isset($add_prod_display) AND ($add_prod_display == 1))) AND $product->minimal_quantity == 1 AND $product->customizable != 2 AND !$PS_CATALOG_MODE}
                                {if ($product->quantity > 0 OR $product->allow_oosp)}
                                    <a class="exclusive ajax_add_to_cart_button" rel="ajax_id_product_{$product->id}"
                                       href="{$link->getPageLink('cart', true, NULL, "qty=1&amp;id_product={$product->id}&amp;token={$static_token}&amp;add")|escape:'html'}"
                                       title="{l s='Add to cart'}"><span></span>{l s='Add to cart'}</a>
                                {else}
                    ...
                    <tr>
                        ...
                        {foreach from=$products item=product name=for_products}
                            ...
                            {if isset($product_features[$product_id])}
                                ...
                                <td width="{$width}%"
                                    class="{$classname} comparison_infos">{if (isset($tab[$feature_id]))}{$tab[$feature_id]|escape:'htmlall':'UTF-8'}{/if}</td>
                            {else}
