                    ...
                    <div class="layered_{$filter.type}" style="display: none;">
                        ...
                        <div>
                            ...
                            <ul id="ul_layered_{$filter.type}_{$filter.id_key}">
                                    ...
                                    <li class="nomargin {if $smarty.foreach.fe.index >= $filter.filter_show_limit}hiddable{/if}">
                                        {if isset($filter.is_color_group) && $filter.is_color_group}
                                            <input class="color-option {if isset($value.checked) && $value.checked}on{/if} {if !$value.nbr}disable{/if}"
                                                   type="button" name="layered_{$filter.type_lite}_{$id_value}"
                                        ...
                                        <label for="layered_{$filter.type_lite}_{$id_value}"{if !$value.nbr} class="disabled"{else}{if isset($filter.is_color_group) && $filter.is_color_group} name="layered_{$filter.type_lite}_{$id_value}" class="layered_color" rel="{$id_value}_{$filter.id_key}"{/if}{/if}>
                                            ...
                                            {else}
                                                <a href="{$value.link}"
                                                   rel="{$value.rel}">{$value.name|escape:html:'UTF-8'}
                                                    {*{if $layered_show_qties}*}
                                                    {*<span> ({$value.nbr})</span>*}
                                                    {*{/if}*}
                                                </a>
                                    ...
                                    <li class="nomargin {if $smarty.foreach.fe.index >= $filter.filter_show_limit}hiddable{/if}">
                                        {if isset($filter.is_color_group) && $filter.is_color_group}
                                            ...
                                            {if isset($value.checked) && $value.checked}<input type="hidden"
                                                                                               ...
                                                                                               value="{$id_value}" />{/if}
                                        {else}
                                            <input type="radio" class="radio layered_{$filter.type_lite}_{$id_value}"
                                        ...
                                        <label for="layered_{$filter.type_lite}_{$id_value}"{if !$value.nbr} class="disabled"{else}{if isset($filter.is_color_group) && $filter.is_color_group} name="layered_{$filter.type_lite}_{$id_value}" class="layered_color" rel="{$id_value}_{$filter.id_key}"{/if}{/if}>
                                            {if !$value.nbr}
                                                {$value.name|escape:html:'UTF-8'}{if $layered_show_qties}<span>
                                                ({$value.nbr})</span>{/if}</a>
                                            {else}
                                                <a href="{$value.link}"
                                                   rel="{$value.rel}">{$value.name|escape:html:'UTF-8'}{if $layered_show_qties}
                                ...
                                {/foreach}
                                {else}
                                    <select class="select" {if $filter.filter_show_limit > 1}multiple="multiple"
                                ...
                                {if $filter.filter_type == 0}
                                    ...
                                    <span id="layered_{$filter.type}_range"><label for="from">От</label><input
                                                type="text" id="from"><label for="to">До</label><input
                                                type="text" id="to"></span>
                                    ...
                                    <script type="text/javascript">
                                        ...
                                        addSlider('{/literal}{$filter.type}{literal}', {
                                            ...
                                            slide: function (event, ui) {
                                                ...
                                                {if $filter.format < 5}
                                                {literal}
                                                from = blocklayeredFormatCurrency(ui.values[0], {/literal}{$filter.format}{literal}, '');
                                                to = blocklayeredFormatCurrency(ui.values[1], {/literal}{$filter.format}{literal}, '');
                                                {/literal}
                                                {else}
                                                {literal}
                                                ...
                                                {/if}
                                                {literal}
                                                $('#layered_{/literal}{$filter.type}{literal}_range').find('#from').val(from);
                                                $('#layered_{/literal}{$filter.type}{literal}_range').find('#to').val(to);
                                            },
                                            stop: function () {
                                                reloadContent();
                                            }
                                        }, '{/literal}{$filter.unit}{literal}', {/literal}{$filter.format}{literal});
                                        var $slider = $('#layered_price_slider'),
                                                $lower = $('#from'),
                                                $upper = $('#to');
                                        $lower.keyup(function () {
                                            var low = $lower.val().replace(' ', ''),
                                                    high = $upper.val().replace(' ', '');
                                            if (low >= {/literal}{$filter.min}{literal} && low < {/literal}{$filter.max}{literal}) {
                                                $lower.val(low);
                                        ...
                                        $upper.keyup(function () {
                                            var low = $lower.val().replace(' ', ''),
                                                    high = $upper.val().replace(' ', '');
                                            if (high <= {/literal}{$filter.max}{literal} && high > {/literal}{$filter.min}{literal}) {
                                                $upper.val(high);
                                ...
                                {if $filter.filter_type == 1}
                                    <li class="nomargin">
                                        {l s='From' mod='blocklayered'} <input
                                                class="layered_{$filter.type}_range layered_input_range_min layered_input_range"
                                                id="layered_{$filter.type}_range_min" type="text"
                                                value="{$filter.values[0]}"/>
                                        <span class="layered_{$filter.type}_range_unit">{$filter.unit}</span>
                                        {l s='to' mod='blocklayered'} <input
                                                class="layered_{$filter.type}_range layered_input_range_max layered_input_range"
                                                id="layered_{$filter.type}_range_max" type="text"
                                                value="{$filter.values[1]}"/>
                                        <span class="layered_{$filter.type}_range_unit">{$filter.unit}</span>
                                        <span class="layered_{$filter.type}_format"
                                              style="display:none;">{$filter.format}</span>
                                        <script type="text/javascript">
                                            {literal}
