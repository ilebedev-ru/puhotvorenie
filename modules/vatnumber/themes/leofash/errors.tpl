...
{if isset($errors) && $errors}
	<div class="error">
		...
		{if isset($smarty.server.HTTP_REFERER) && !strstr($request_uri, 'authentication') && preg_replace('#^https?://[^/]+/#', '/', $smarty.server.HTTP_REFERER) != $request_uri}
			<p class="lnk"><a href="{$smarty.server.HTTP_REFERER|escape:'htmlall':'UTF-8'|secureReferrer}" title="{l s='Back'}"><i class="fa fa-angle-double-left" aria-hidden="true"></i> {l s='Back'}</a></p>
		{/if}
	</div>
{/if}