...
{else}
	...
	<div id="cart_voucher" class="table_block">
		...
		<form action="{if $opc}{$link->getPageLink('order-opc.php', true)}{else}{$link->getPageLink('order.php', true)}{/if}" method="post" id="voucher">
			<label for="discount_name">{l s='Укажите код купона:'}</label>
			...
			<a class="fancy_modal iframe link" href="/content/10-kak-poluchit-kupon-na-sleduushhuu-pokupku?content_only=1">Как получить купон на следующую покупку?</a>
		</form>
	</div>
	{/if}
	
	{if !empty($HOOK_SHOPPING_CART)}<div id="HOOK_SHOPPING_CART">{$HOOK_SHOPPING_CART}</div>
	{else}
