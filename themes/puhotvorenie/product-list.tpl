...
<div class="catalog nuclear">
	...
	<div class="item">
		<span class="img-wrp"><a{if isset($product.link)} href="{$product.link}"{/if} title="{$product.name|escape:html:'UTF-8'}" class="product_image">
		...
		<a href="{$product.link}" class="name">{$product.name|escape:'htmlall':'UTF-8'}</a>
		  <span class="desc">{$product.description_short|strip_tags}</span>
