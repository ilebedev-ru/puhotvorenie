...
{*{if $smarty.server.HTTP_REFERER && strpos($smarty.server.HTTP_REFERER, '//'|cat:$smarty.server.SERVER_NAME)}
	<a href="{$smarty.server.HTTP_REFERER}">← Вернуться к покупкам</a><br><br>
{/if}*}
<a href="/">← Вернуться к покупкам</a><br><br>

<h1 id="cart_title">{l s='Корзина'}</h1>
...
$(
	function()
	{
		...
		{
			var $cost;
			var $carrier=$('.deliv-type input:checked');
			if($carrier.length>0)
			{
				$carrier=$($carrier[0].parentNode.parentNode);
				$cost=$carrier.find('.cost').html();
				$('#id_delivery_line').show();
				$('#id_delivery_line_value').html($cost);
			}
			else
			{
				$('#id_delivery_line').hide();
			}
		}
		
		changeDelivery();
		$('.deliv-type').click(changeDelivery);
...
<div id="order-detail-content" class="table_block">
	<table id="cart_summary" class="std table-bordered">
		...
		<tfoot>
			<tr id="id_delivery_line" style="display:none">
				<td colspan="3"></td>
				<td colspan="2" style="text-align:right">{l s='Доставка'}:&nbsp;<span id="id_delivery_line_value"></span></td>
			</tr>
			<tr class="cart_total_price">
				<td colspan="3" id="cart_voucher"  >
				{if $voucherAllowed && 0}
	...
	{if sizeof($discounts)}
		...
		{foreach $discounts as $discount}
			<tr class="cart_discount {if $discount@last}last_item{elseif $discount@first}first_item{else}item{/if}" id="cart_discount_{$discount.id_discount}">
				...
				<td class="cart_discount_price total_discount_price">
