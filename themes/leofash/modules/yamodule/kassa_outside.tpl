...
{if $DATA_ORG && $DATA_ORG['YA_ORG_ACTIVE'] && $summ >= $DATA_ORG['YA_ORG_MIN']}
	<div class="row">
		<div class="col-xs-12">
			{if $yandex_logo}
				<a href="{$link->getModuleLink('yamodule', 'redirectk', [], true)|escape:'quotes':'UTF-8'}"
				   title="{l s='Оплатить через Яндекс' mod='yamodule'}">
			...
			{if !$yandex_logo}
				<p class="payment_module">
					<a class='ym_AC' href="{$link->getModuleLink('yamodule', 'redirectk', [], true)|escape:'quotes':'UTF-8'}" title="{l s='Оплата на стороне сервиса Яндекс.Касса' mod='yamodule'}">
						<img
							 src="{$img_dir|escape:'htmlall':'UTF-8'}yakassa.jpg" />{fd($summary)}
						{if $summary.total_discounts > 0}
                            {l s='Оплата на стороне сервиса Яндекс.Касса' mod='yamodule'}
						{else}
                            {l s='Оплата онлайн со скидкой' mod='yamodule'} {$percent|string_format:'%d'}%
						{/if}
						- {convertPrice price=$total_to_pay}
					</a>
