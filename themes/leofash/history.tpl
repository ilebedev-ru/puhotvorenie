	...
	<table id="order-list" class="std table-bordered">
		...
		{foreach from=$orders item=order name=myLoop}
			<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
				...
				<td class="history_price"><span>{displayPrice price=$order.total_paid currency=$order.id_currency no_utf8=false convert=false}</span></td>
...
<ul class="footer_links clearfix">
	<li class="f_left"><a href="{$link->getPageLink('my-account', true)|escape:'html'}"><span class="fa fa-user"></span>{l s='Back to Your Account'}</a></li>
	<li class="f_right"><a href="{$base_dir}"><span class="fa fa-home"></span>{l s='Home'}</a></li>
</ul>
